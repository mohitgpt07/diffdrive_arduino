  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  // wheel_rl_.enc = wheel_fl_.enc;
  // wheel_rr_.enc = wheel_fr_.enc;
  ...
  double pos_prev = wheel_fl_.pos;
  wheel_fl_.pos = pos_prev + wheel_fl_.cmd * delta_seconds;
  wheel_fl_.vel = wheel_fl_.cmd;

  pos_prev = wheel_fr_.pos;
  wheel_fr_.pos = pos_prev + wheel_fr_.cmd * delta_seconds;
  wheel_fr_.vel = wheel_fr_.cmd;

  pos_prev = wheel_rl_.pos;
  wheel_rl_.pos = pos_prev + wheel_rl_.cmd * delta_seconds;
  wheel_rl_.vel = wheel_rl_.cmd;

  pos_prev = wheel_rr_.pos;
  wheel_rr_.pos = pos_prev + wheel_rr_.cmd * delta_seconds;
  wheel_rr_.vel = wheel_rr_.cmd;
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  // int motor_l_counts_per_loop = (wheel_fl_.cmd + wheel_rl_.cmd)/ wheel_fl_.rads_per_count / cfg_.loop_rate;
  // int motor_r_counts_per_loop = (wheel_fr_.cmd + wheel_rr_.cmd)/ wheel_fr_.rads_per_count / cfg_.loop_rate;
  // comms_.set_motor_values(motor_l_counts_per_loop, motor_r_counts_per_loop);

  double max_wheel_vel = 36.2;
  int motor_l_raw_pwm = (wheel_fl_.cmd + wheel_rl_.cmd) * 255 / (2 * max_wheel_vel);
  int motor_r_raw_pwm = (wheel_fr_.cmd + wheel_rr_.cmd) * 255 / (2 * max_wheel_vel);
  comms_.set_motor_values(motor_l_raw_pwm, motor_r_raw_pwm);
